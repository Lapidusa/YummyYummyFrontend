<template>
  ...
  <div v-else class="overflow-y-auto">
    <p><b>Id:</b> {{ thisCategory?.id }}</p>
    <p><b>Категория:</b> {{ thisCategory?.name }}</p>
    <p><b>Описание:</b> {{ thisCategory?.description }}</p>
    <div v-if="thisCategory.products.length !== 0">
      <p><b>Продукты:</b></p>
      <div class="flex flex-wrap gap-2">
        <router-link :to="`/admin/store/category/product/${product.id}`" class="btnLogOut flex" v-for="product in thisCategory?.products">
          <div class="container">
    ...
    <div class="flex gap-3">
      <div class="btn" type="button" @click="openModalCreate(false)">
        Создать продукт
      </div>
      <div class="btn" type="button" @click="openModalCreate(true)">
        Изменить категорию
      </div>
      <div class="btn" type="button" @click="deleteCategory()">
        Удалить категорию
      </div>
    </div>
  </div>
  ...
  >
    ...
    <template v-if="!changeModalContent">
      <div class="bg-white rounded-lg shadow-lg p-6 z-50 relative">
        <h2 class="text-3xl font-semibold mb-4">Создать продукт в категории: {{ thisCategory?.name }}</h2>
        ...
        <button @click="createProduct" class="btnLogIn w-full justify-center">
          <p class="btn">Создать продукт</p>
        </button>
    ...
    <template v-else>
      ...
      <div class="bg-white rounded-lg shadow-lg p-6 w-96 z-50 relative">
        <h2 class="text-3xl font-semibold mb-4 ">Изменить категорию</h2>
        <div class="mb-4">
          <label for="name" class="block text-sm font-medium text-gray-700">Название категории</label>
          <input type="text" id="name" v-model="thisCategory.name" class="mt-1 block w-full border border-gray-300 rounded-md shadow-sm focus:ring focus:ring-opacity-50" placeholder="Введите название продукта"/>
        </div>
        <div class="mb-4">
          <label for="description" class="block text-sm font-medium text-gray-700">Описание категории</label>
          <input type="text" id="description" v-model="thisCategory.description" class="mt-1 block w-full border border-gray-300 rounded-md shadow-sm focus:ring focus:ring-opacity-50" placeholder="Введите описание продукта"/>
        </div>
        <button @click="updateCategory" class="btnLogIn w-full justify-center">
...
import {CreateCategory, CreateProduct, DeleteCategory, getThisCategory, UpdateCategory} from "~/server/responsesAPI.js";
...
const ingredients = reactive([{ name: '', canRemove: false }]);
const pizzaVariants = reactive([{ size: 0, price: 0, energyValue: 0, proteins: 0, fats: 0, carbohydrates: 0, weight: 0 }]);
const isPizza = ref(false);
const productVariants = reactive([{ name: '', price: 0, energyValue: 0, proteins: 0, fats: 0, carbohydrates: 0, weight: 0 }]);
const fileInput = ref(null);

const getCategory = async () => {
  try {
    const res = await getThisCategory(categoryId);
    if (res && res.category) {
      thisCategory.value = res.category;
      storeId.value = res.category.storeId
    } else {
...
const addPizzaVariant = () => {
  pizzaVariants.push({ size: 0, price: 0, energyValue: 0, proteins: 0, fats: 0, carbohydrates: 0, weight: 0 });
};
...
const updateCategory = async () =>{
  const res = await UpdateCategory(categoryId, storeId.value, thisCategory.value.name, thisCategory.value.description)
  await getCategory();
  if (res.result) {
    closeModalCreate()
  }
...
const deleteCategory = async () =>{
  let confirmDelete = confirm(`Вы точно хотите удалить категорию ${thisCategory.value.name}? Продукты удаляться тоже.`);
  if (confirmDelete ) {
